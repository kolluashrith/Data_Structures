...
The load factor does significantly affect performance because it affects the frequency of collisions and affects the frequency at which the grow function is called. Though there are some outliers, across the datasets it appears that Open Addressing does typically perform better with lower load factors. This makes sense because lower load factors prevents dense arrays, reducing the collisions and decreasing the number of cells that need to be searched on average to find an empty slot for insertion. There are tests in which Open Addressing does well with a relatively high load factor of 0.9, which could be attributed to the datasets happening to cause an unusually low collision frequency. The differences matter a lot for open addressing. With random164.txt, for example, the difference between the performance with a load factor of 0.6 vs 0.9 was almost a full second -- about 350% -- slower. This difference in performance based on load factor is visible with chaining as well, but in the opposite direction. The results suggest that higher load factors result in faster runtimes than lower ones. This makes sense for chaining because higher load factors prevent frequent grow operations, which are usually more time-consuming than iterating through the relatively short number of elements in a given ArrayList that is expected from collisions.

New question: Which hash table implementation has the most stable performance across load factors?
From the results, it appears that chaining is more stable than open addressing. In other words, the open addressing implementation performance is more sensitive to the choice of load factor than chaining is. With random164.txt, we noticed 350% slower performance between the best and worst load factor choice. However, with chaining, the difference between the best and worst was only a 67% slower runtime. Similar differences in performance drops are evident across the other datasets tested, suggesting that chaining is more robust as a hash table implementation and has more consistent performance across load factors. Since it can handle higher load factors better as well (it can even handle load factors over 1 but we did not test that here), where space is an issue, chaining might be beneficial to prevent excessive memory consumption. 

Overall, open addressing is likely the better implementation if space is not a concern and runtime performance matters greatly. It might need to be optimized for the dataset used, but it will likely result in faster times. On the other hand, if a more robust implementation is needed or if space is a concern, chaining is a good alternative. It keeps relatively fast runtime balanced with less space consumption and robust performance across various load factors. 